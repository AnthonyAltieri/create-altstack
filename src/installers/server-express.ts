...
export const serverExpressInstaller = ({
  projectDir,
  projectName,
  options,
  ...
  // Copy the server-express template
  fs.copySync(path.join(extrasDir, "apps/server-express"), serverAppDir);
  ...
  addPackageDependency({
    projectDir: serverAppDir,
    dependencies: ["typescript", "@types/node", "@types/express", "tsx"],
    devMode: true,
  });
};
